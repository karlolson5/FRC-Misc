# Tipping Stability, Zero Moment Point, Wheel Normal Force, Friction, and Swerve Drive Controller Architecture

This document derives, in one continuous pass: (1) the tipping/ZMP stability criterion for a wheeled robot, (2) the distribution of per-wheel normal forces, (3) the corresponding friction/traction force problem, and (4) a swerve-drive controller architecture that ties all of the above together — including the effect of internally moving masses (arms, elevators, slides) on stability and required wheel forces. Section numbers are global and consistent throughout, so later parts freely reference earlier derivations.

---

# Part I — Tipping Stability, Zero Moment Point, and Wheel Normal Force Derivation
...
## 4. Tipping Criterion (intuitive edge-wise test)

For a candidate tipping edge $e_k$ of the polygon $\mathcal P$, connecting adjacent wheels $r_k^w, r_{k+1}^w$ (CCW-ordered, $r_{k+1}^w \to r_1^w$ for $k=N_w$):

$$
\hat e_k \propto r_{k+1}^w - r_k^w \quad\text{(edge direction)}, \qquad \hat n_k = \hat z \times \hat e_k \quad\text{(outward horizontal normal)}
$$

Pick $P_k$ on edge $e_k$. Wheels **on** the edge contribute nothing to the moment about $\hat e_k$ (their moment arm is parallel to $\hat e_k$). Wheels **off** the edge (interior side, since $\mathcal P$ convex) contribute only non-negative amounts (proportional to $F_j^N \ge 0$).

Define the **tipping moment**:

$$
\tau_P \equiv \Big[\sum_i (r_i-P)\times m_i(\vec g - a_i)\Big]_{xy}\cdot \hat e_k
$$

**Tipping condition:**

$$
\boxed{\text{Robot tips about edge } e_k \iff \tau_P > 0}
$$

(If $\tau_P > 0$, no non-negative distribution of off-edge $F_j^N$ can balance the equation — those wheels lift off.)

Check this over all $k=1 \dots N_w$ edges; robot is stable iff none trip the inequality.
...
**Derivation:** with $r_i - P = (x_i - x,\ y_i - y,\ z_i)$, i.e., for a point $P$ on the $x-y$ plane, the moment components are:
...
**Degenerate case:** if $\sum_i N_{iz} \le 0$ (e.g. free-fall or accelerating off the ground), $r_{zmp}$ is ill-defined; handle as a separate case (all wheels unloaded).
...
# Part II — Wheel Friction / Traction Force Derivation
...
## 10. Indeterminacy — worse than the normal-force case
...
## 11. Closing the System

### (a) Compliance model (tangential stiffness)

Analogous to Section 6(a). Model each wheel/tire contact patch with tangential stiffness $k_j^t$ and a rigid-body horizontal + yaw deflection field:

$$
f_j = k_{t,j}\, \delta_j^{\,t}, \qquad \delta_j^{\,t} = \delta_0^{\,t} + \vec\omega_\delta \times (r_j^w - \bar r^w)
$$

where $\delta_0^{\,t}$ is a common horizontal shear deflection and $\vec\omega_\delta = \omega_\delta \hat z$ a small yaw twist. Substituting into the 3 equilibrium equations (Sections 8–9) gives 3 equations for the 3 unknowns $(\delta_{0,x}^{\,t}, \delta_{0,y}^{\,t}, \omega_\delta)$, which then determine all $f_j$ uniquely — same structure as the equal-stiffness normal-force fit, just with an added rotational (yaw) degree of freedom since the tangent plane allows in-plane torsion.

### (b) Minimum-norm / optimization solve

$$
\min_{f_j} \sum_j \|f_j\|^2 \quad \text{s.t. Sections 8–9 equalities}
$$
...
$$
\min_{f_j} \sum_j \|f_j\|^2 \quad \text{s.t. Sections 8–9 equalities}, \quad \|f_j\| \le \mu_j F_j^N \ \forall j
$$
...
Same idea as Section 6(a′) for normal forces: if a compliance solution to (a) yields $\|f_j\| > \mu_j F_j^N$ at some wheel, that wheel is saturated (slipping), not sticking. Clamp it to the cone boundary $f_j = \mu_j F_j^N \, \hat f_j$ (direction from the unconstrained solve), remove it from the "sticking" active set, and re-solve the compliance fit over the remaining wheels — iterate to convergence, same termination logic as before.


---

# Part III — Advanced Swerve Drive Controller Architecture (with Internal Mass Motion)

## 12. Internal Mass Accelerations

Each mass element's acceleration has **two independent contributions**:

$$
a_i(\vec u, t) = a_i^{rigid}(\vec u) + a_i^{int}(t)
$$

- $a_i^{rigid}(\vec u)$ — from the chassis's rigid-body motion (translation + yaw + centripetal), **affine in the commanded** $\vec u=(a_x,a_y,\alpha_{yaw})$.
...
**Consequence:** every downstream quantity is **affine but not linear** in $\vec u$ (it has a nonzero offset at $\vec u=0$, coming from internal-mass reaction effects). All of the proportional-scaling logic developed below (Section 14) still works, but $s=0$ no longer means "no net force/tipping effect" — it means "purely the reaction loads from internal mechanisms." This is physically correct: a fully extended, rapidly retracting arm can shift $r_{zmp}$ or demand wheel torque even while the chassis itself is commanded to zero acceleration.


## 13. Restating the Givens

**Fixed/known:** $r_i, m_i$ (mass elements — note $r_i$ may itself be time-varying, e.g. the extending arm's mass position), wheel data $r_j^w, R_j, \mu_j, k_j$ (compliance spring constants for normal-force distribution, Section 6a), torque limits $\tau_j^{max}$.
...
This makes every downstream quantity ($r_{zmp}$, $F_j^N$, required wheel wrench) an explicit function of $\vec u$ (and, through $a_i^{int}$, of the current mechanism state), which is what lets the controller scale $\vec u$ down later while leaving $a_i^{int}$ untouched.
...
```
 u_cmd = (ax, ay, α)
        │
        ▼
 [1] Rigid-body + internal kinematics  →  a_i(u,t) = a_i^rigid(u) + a_i^int(t)
        │
        ▼
 [2] ZMP / tipping check    →  s_tip  (max safe scale, tipping alone)
        │
        ▼
 [3] Normal force solve     →  F_j^N(u,t)   (Section 6, compliance + active-set)
        │
        ▼
 [4] Per-wheel force cap    →  F_j^max(u,t) = min(μ_j F_j^N, τ_j^max / R_j)
        │
        ▼
 [5] Required wrench        →  F_req(u,t), τ_req(u,t)   (rigid-body Newton–Euler + internal reaction)
        │
        ▼
 [6] Wrench allocation SOCP →  f_j , s_force  (max achievable fraction)
        │
        ▼
 [7] Combine: s = min(s_tip, s_force, 1); recompute at s·u_cmd (a_i^int unscaled)
        │
        ▼
 [8] Output: θ_j = atan2(f_j), τ_j = |f_j|·R_j
 ```

The key structural idea: **stages 2–6 are all affine functions of a single scalar $s$ applied to $\vec u_{cmd}$**, because $a_i^{rigid}$ is affine in $\vec u$ and $a_i^{int}$ is a fixed (per-cycle) offset that does not depend on $s$. Everything downstream (tipping margin, weight transfer, required wrench, friction budget) inherits that affine-in-$s$ dependence, which is what makes "reduce the magnitude of desired chassis acceleration proportionally" well-posed as a single 1-D search — the internal-mechanism contribution simply becomes a nonzero intercept at $s=0$ rather than vanishing there.


## 15. Stage Details

### 15.1 Tipping limit → $s_{tip}$

$N_{iz} = m_i(-g - a_{i,z})$; since $a_i^{int}$ can have a $z$-component (elevator!), $W_{eff}=\sum_i N_{iz}$ **may itself depend on internal motion**, but critically it is still **independent of $s$** (the chassis command scale), since $a_i^{rigid}$ has no $z$-component for a planar rigid chassis. So the affine-in-$s$ structure survives:

$$
r_{zmp}(s) = r_{zmp}(0) + s\big[r_{zmp}(1) - r_{zmp}(0)\big]
$$

a straight line in $s\in[0,1]$ from the static-chassis ZMP to the fully-commanded ZMP. But now $r_{zmp}(0)$ (the $s=0$ intercept) reflects the **static-chassis, moving-mechanism** ZMP — e.g. a raised elevator or extended arm decelerating can by itself push $r_{zmp}(0)$ outside $\mathcal P$, meaning **the robot can tip with zero commanded drive acceleration**, purely from arm/elevator motion.

Finding $s_{tip}$ = the largest $s$ keeping $r_{zmp}(s)\in\mathcal P$ is a single **ray–polygon intersection** — closed form, no iteration needed (include a safety margin by shrinking $\mathcal P$ inward before the test). If $r_{zmp}(0)\notin\mathcal P$ already, $s_{tip}=0$ (or negative — flag as an unrecoverable-by-drive tipping event, since no amount of chassis acceleration scaling fixes a problem that exists at zero chassis command).

### 15.2 Normal force distribution → $F_j^N(s)$

Using $a_i(s) = s\,a_i^{rigid}(1) + a_i^{int}(t)$, run Section 6's compliance model with the given $k_j$, plus the active-set correction (Section 6(a′)) for $N_w > 3$. Internal mechanism motion contributes to weight transfer independent of drive commands (e.g. an arm swinging redistributes wheel loads), and because horizontal acceleration commands also shift $r_{zmp}$ within $\mathcal P$, $F_j^N$ is *not* constant in $s$ even for $s \le s_{tip}$ — heavier braking/turning still loads some wheels more than others well before tipping occurs. This matters for the friction budget next.

### 15.3 Per-wheel force capacity → $F_j^{max}(s)$
...
This sidesteps the friction-force indeterminacy problem from Part II (Section 10) — there we needed a *compliance* model because the forces were passive/reactive. Here the wheels are **actuated**, so instead of a physical compliance law we get to choose $f_j$ via optimization, subject only to this magnitude cap (an isotropic disk constraint per wheel).

### 15.4 Required wrench → $F_{req}(s), \tau_{req}(s)$

The drivebase must react against **both** the commanded rigid-body motion **and** the momentum change of internally-moving masses (e.g. an extending arm's reaction force is transmitted to the chassis, and from the chassis to the ground through the wheels). Using Newton's second law and $\tau=dL/dt$ for the *whole* robot about $r_{cm}$:

$$
F_{req}(s) = \sum_i m_i\,a_i(s) = \underbrace{s\,M_{tot}\,(a_x,a_y)}_{\text{rigid-body term, }F_{req}^{rigid}} + \underbrace{\sum_i m_i\, a_i^{int}(t)}_{\text{internal reaction force, }F_{req}^{int},\ \text{indep. of } s}
$$

$$
\tau_{req}(s) = \sum_i (r_i - r_{cm})\times m_i a_i(s) = \underbrace{s\,I_{zz}\,\alpha_{yaw}}_{\text{rigid-body term, }\tau_{req}^{rigid}} + \underbrace{\sum_i (r_i-r_{cm})\times m_i\,a_i^{int}(t)}_{\text{internal reaction moment, }\tau_{req}^{int},\ \text{indep. of }s}
$$

with $M_{tot}=\sum_i m_i$, $I_{zz}=\sum_i m_i \|r_i'\|^2$ (about $r_{cm}$). Both remain **affine in $s$** — same structure as the pure rigid-body case, just with a generally-nonzero intercept at $s=0$. Note that if any $r_i$ is itself moving (e.g. the arm's mass position changing), $I_{zz}$ and even $r_{cm}$ itself are time-varying — recompute both each control cycle from current $r_i(t)$, not cached from a prior cycle.

### 15.5 Wrench allocation (SOCP)
...
\text{s.t.}\quad & \sum_j f_j = F_{req}(s) \\
& \sum_j (r_j^w - r_{cm})\times f_j = \tau_{req}(s) \\
...
### 15.6 Feasibility & proportional scale-back → $s_{force}$

Fold the scale factor directly into the allocation problem rather than solving allocation and *then* checking feasibility — this avoids a separate bisection when possible. Maximize $s$ such that the equality constraints (now with the internal-reaction offset) remain satisfiable within the disk caps:
...
**Caveat (coupling with weight transfer):** $F_j^{max}(s)$ itself depends on $s$ through $F_j^N(s)$ (Section 15.2), so this is not a fixed convex set — it's a *family* of convex sets shrinking/growing with $s$. In practice:
...
| $m_i$ | mass of point $i$ |
| $r_i$ | position of mass $i$ (may be time-varying, e.g. an extending arm) |
| $\vec g$ | gravity vector, $-g\hat z$ |
| $F_j^N$ | normal (vertical) force at wheel $j$, $\ge 0$ |
| $f_j$ | friction/traction (horizontal) force at wheel $j$ |
...
| $F_j^N(s)$ | normal force at wheel $j$ as a function of scale $s$ (Section 6 compliance/active-set) |
| $F_j^{max}(s)$ | per-wheel force cap, $\min(\mu_j F_j^N(s), \tau_j^{max}/R_j)$ |
| $F_{req}, \tau_{req}$ | total required net force / yaw moment |
